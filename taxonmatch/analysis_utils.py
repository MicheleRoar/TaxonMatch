...
def clean_taxon_names(df, column_name):
    """
    Cleans taxonomic names by applying the following general rules:
    - Removes everything inside parentheses (including parentheses).
    - Removes taxonomic prefixes 'subsp.' and 'var.'.
    - Removes all words with an uppercase initial letter (except the first word), even if they contain accented characters or apostrophes.
    ...
    :param column_name: Name of the column to clean (default "TAXON").
    :return: DataFrame with the cleaned TAXON column.
    """
    df = df.copy()  # Prevents unintended modifications to the original DataFrame

    # Removes everything inside parentheses (including parentheses)
    df[column_name] = df[column_name].astype(str).str.replace(r"\s*\(.*?\)", "", regex=True)

    # Removes the prefixes 'subsp.' and 'var.'
    df[column_name] = df[column_name].str.replace(r'\b(subsp|var)\b', '', regex=True)

    # Removes special characters (including ., &, ',', and apostrophe ')
    df[column_name] = df[column_name].str.replace(r"[.,&']", "", regex=True)

    # Normalizes characters to handle accented letters (converting them into standard forms)
    df[column_name] = df[column_name].apply(lambda x: unicodedata.normalize('NFKD', x).encode('ascii', 'ignore').decode('utf-8'))

    # Removes all words with an uppercase initial letter (except the first one), even if they contain special characters or apostrophes
    df[column_name] = df[column_name].apply(
        lambda x: ' '.join([w for i, w in enumerate(x.split()) if i == 0 or not re.match(r"^[A-Z][a-zA-Z-'’]*$", w)])
    )

    # Removes the word 'ex' (which often appears in taxonomic attributions)
    df[column_name] = df[column_name].str.replace(r'\bex\b', '', regex=True)

    # Removes multiple spaces generated by cleaning
